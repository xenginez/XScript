...
struct x::allocator::page
{
	...
	std::atomic<x::allocator::block *>		thread_free = 0;						// list of deferred free blocks freed by other threads
	std::atomic<x::uint64>					thread_freed = 0;
...
{
	static thread_local x::allocator::tld _tld;
...
{
	if ( auto p = get_page( ptr ) )
	{
		auto b = (block *)ptr;

		if ( get_segment( ptr )->thread_id == std::this_thread::get_id() )
		{
			b->next = p->local_free;
			p->local_free = b;
			p->used--;
			if ( p->used == 0 && p->thread_free == nullptr )
				instance()->free_page( p );
		}
		else
		{
			do { b->next = p->thread_free; } while ( !p->thread_free.compare_exchange_weak( b->next, b ) );
		}
	}
}
...
void x::allocator::foreach_used( std::function<void( void * )> && callback )
{
}

x::allocator::heap * x::allocator::get_default_heap()
{
	return private_p::_tld.heaps;
}

x::allocator::page * x::allocator::get_page( void * ptr )
{
	if ( auto seg = get_segment( ptr ) )
